            ...
            if self.data_dir:
                if self.modal == 'path':
                    ...
                    for slide_id in slide_ids:
                        try:
                            wsi_path = os.path.join(data_dir, 'resnet50', '{}.pt'.format(slide_id.rstrip('.svs')))
                    ...
                    for slide_id in slide_ids:
                        try:
                            ...
                            continue
                    path_features = torch.cat(path_features, dim=0) if len(path_features)>0 else torch.zeros((1,1024))
                    if self.OOM > 0 and path_features.size(0) > self.OOM:
                        path_features = path_features[np.random.choice(path_features.size(0), self.OOM, replace=False)]
                    
                    # Extract genomic features
                    if self.signatures is not None and hasattr(self, 'omic_names'):
                        genomic_features = torch.cat([torch.tensor(self.genomic_features[omic_name].iloc[idx].values, dtype=torch.float32) for omic_name in self.omic_names])
                    else:
                        genomic_features = torch.tensor(self.genomic_features.iloc[idx].values, dtype=torch.float32)
                    
                    # Extract clinical metadata features dynamically
                    # Getting the non-genomic columns (e.g. age, sex, stage, grade)
                    clinical_data = []
                    for m_col in self.metadata:
                        val = self.slide_data.iloc[idx][m_col]
                        # Super simple auto-encoder for clinical data (convert to floats)
                        try:
                            # Use numeric if possible
                            num_val = float(val)
                            if np.isnan(num_val) or np.isinf(num_val):
                                num_val = 0.0
                        except:
                            # if it's string or categorical, we just hash it roughly or map to codes
                            # adding a small offset to avoid 0 if needed
                            num_val = float(hash(str(val)) % 1000) / 1000.0
                        clinical_data.append(num_val)
                    clinical_features = torch.tensor(clinical_data, dtype=torch.float32)
                    
                    return (path_features, genomic_features, clinical_features, label, event_time, c)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, modal, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2):
        ...
        self.slide_cls_ids = [[] for i in range(self.num_classes)]
        self.OOM = 0 # Default to 0 for split, or could pull kwargs from father init
    ...
    def apply_scaler(self, scalers: tuple = None):
        transformed = pd.DataFrame(scalers[0].transform(self.genomic_features.fillna(0)))
        transformed.columns = self.genomic_features.columns
        self.genomic_features = transformed.fillna(0)
    # <--
