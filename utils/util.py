...
def collate_MIL_survival_trimodal(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    omic = torch.stack([item[1] for item in batch], dim = 0).type(torch.FloatTensor)
    clin = torch.stack([item[2] for item in batch], dim = 0).type(torch.FloatTensor)
    label = torch.LongTensor([item[3] for item in batch])
    event_time = np.array([item[4] for item in batch])
    c = torch.FloatTensor([item[5] for item in batch])
    return [img, omic, clin, label, event_time, c]
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, modal='coattn', batch_size=1):
    ...
    if modal == 'coattn':
        ...
        collate = collate_MIL_survival_trimodal
    else:
